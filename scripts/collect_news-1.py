...
def parse_cse_date(snippet: str) -> date | None:
    ...
    if not snippet:
        return None
    # 中文格式：2026年5月14日 或 2026年04月14日
    m = re.search(r'(\d{4})年\s*(\d{1,2})月\s*(\d{1,2})日', snippet)
    if m:
        try:
            return date(int(m.group(1)), int(m.group(2)), int(m.group(3)))
        except ValueError:
            pass
    # 英文格式：May 14, 2026
    m = re.search(r'([A-Z][a-z]{2})\s+(\d{1,2}),\s+(\d{4})', snippet)
    if m:
        try:
...
def fetch_cse(keyword: str, category: str, priority: int, sites: list) -> list:
    ...
    params = {
        ...
        "num": 5,
        "dateRestrict": "d7",   # 改為 7 天限制（原本 d1 但 Google 不嚴格遵守）
        "lr": "lang_zh-TW"
    }
    items = []
    try:
        ...
        for item in r.json().get("items", []):
            ...
            # 從 snippet 萃取真實發布日期
            pub_date = parse_cse_date(snippet)

            # 若能解析日期且超過截止日，跳過
            if pub_date and pub_date < CUTOFF_DATE:
                print(f"  [CSE 過濾舊文] {item.get('title','')[:40]} ({pub_date})")
                continue

            # 若無法從 snippet 解析日期，使用 TODAY 作 fallback
            published_str = pub_date.isoformat() if pub_date else TODAY
...
def main():
    all_items = []
    print(f"=== 開始蒐集 {TODAY}（過濾 {MAX_AGE_DAYS} 天前的文章）===")
