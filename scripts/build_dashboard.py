...
/* pub_date 最大值 / 最小值 — 所有日期邏輯的唯一依據 */
function getLatestPubDate() {
  let d = '';
  allItemsFlat.forEach(x=>{ const p=normalizeDate(x.pub_date||x.published||x.date||''); if(p&&p>d) d=p; });
  return d;
}
...
function init() {
  ...
  document.getElementById('statTotal').textContent = todayItems.length;
  document.getElementById('statNeg').textContent   = todayItems.filter(x=>getSentiment(x)==='負面').length;
  document.getElementById('statPos').textContent   = todayItems.filter(x=>getSentiment(x)==='正面').length;
  updateTrackKPI();

  // ④ Header 資料庫範圍
  const elR = document.getElementById('hdrDateRange');
  if (elR && earliest) elR.innerHTML = toROC(earliest) + '<br>至 ' + toROC(latest);
  ...
  // ⑤ 預設日期篩選：近30天（以 pub_date 最大值往回推）
  const thirtyBack = new Date(new Date(latest+'T00:00:00').getTime()-30*86400000).toISOString().slice(0,10);
  document.getElementById('dateFrom').value = (earliest && thirtyBack < earliest) ? earliest : thirtyBack;
...
function updateTrackKPI() {
  ...
  if (el) {
    ...
    el.textContent = allItemsFlat.filter(x=>tracked[getItemId(x)]).length;
  }
}
...
function renderTodayNeg(latest) {
  const items = allItemsFlat.filter(x=>getSentiment(x)==='負面' && normalizeDate(x.pub_date||x.published||x.date||'')===latest);
...
function archiveToForm(id) {
  ...
  renderClarifyList();
  renderTodayNeg(getLatestPubDate());
}
...
async function loadFromSheets() {
  ...
  try {
    const resp = await fetch(_SHEETS_GET_URL + '?action=getAll', {redirect:'follow', signal: AbortSignal.timeout(40000)});
