...
.cat-ring { border-radius:50%; display:flex; flex-direction:column; align-items:center; justify-content:center; color:white; font-weight:700; transition:box-shadow .2s; }
.cat-name { font-size:.72rem; font-weight:600; margin-top:.5rem; text-align:center; max-width:80px; line-height:1.3; }
/* Chip */
    ...
    <div class="flex justify-between items-center gap-4 flex-wrap">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <div class="text-blue-400 text-xs mt-0.5">🕐 每日 07:00 自動蒐集分析</div>
        </div>
      </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-5 gap-3">
    ...
    <div class="lg:col-span-2 bg-gradient-to-br from-red-50 to-orange-50 border border-red-200 rounded-xl p-3.5">
      <div class="flex items-center justify-between mb-3">
        <div class="flex items-center gap-2">
          ...
          <span class="font-bold text-red-700 text-sm">今日緊急預警</span>
        </div>
        <span class="text-xs text-red-400 font-medium" id="urgentCount"></span>
      </div>
      <div id="urgentList" class="space-y-2.5"></div>
      <div id="urgentEmpty" class="hidden text-center py-6">
        <div class="text-3xl mb-2">✅</div>
        <div class="text-green-600 text-xs font-medium">今日無高優先負面輿情</div>
      </div>
    </div>
    ...
    <div class="lg:col-span-3 bg-white border border-slate-100 rounded-xl shadow-sm">
      ...
      <div id="clarifyEmpty" class="hidden text-center py-6 text-slate-400">
        ...
        <div class="text-xs">尚無追蹤中的輿情<br>在新聞列表點擊「追蹤」即可加入</div>
      </div>
    </div>
  ...
  <div class="layout-main" style="display:grid;grid-template-columns:248px 1fr;gap:16px;align-items:start">
    ...
    <aside class="layout-sidebar sidebar rounded-xl shadow-xl" style="position:sticky;top:12px">
      <div class="sidebar-scroll">
        ...
        <!-- 輿情分析 -->
        <div class="sb-sec">
          <span class="sb-title">📊 近30天輿情分析</span>
          ...
          <div class="mt-3 grid grid-cols-2 gap-1.5 text-xs">
            <div class="text-center py-1 rounded" style="background:#1a2e4a;color:#7eb4d8">
    ...
    <main class="space-y-3">
      ...
      <div class="bg-white rounded-xl shadow-sm p-5 border border-slate-100">
        <div class="flex items-center justify-between mb-4">
          <h3 class="font-bold text-slate-700 text-sm">📂 分類總覽</h3>
          <button onclick="selectCat(null,null)" class="text-xs text-blue-400 hover:text-blue-600">顯示全部</button>
        </div>
        <div class="flex justify-around items-start gap-2" id="catCircles"></div>
      </div>
...
// 4 分類對應
const CAT_GROUPS = {
  '南水資源分署':   {cats:['南部水資源','社群輿情'], color:'#1e5799', grad:'linear-gradient(135deg,#1a3a6c,#2d6a9f)'},
  '台南海水淡化廠': {cats:['海淡廠'],                color:'#065f46', grad:'linear-gradient(135deg,#064e3b,#059669)'},
  '水庫相關新聞':   {cats:['全台水資源'],             color:'#92400e', grad:'linear-gradient(135deg,#78350f,#d97706)'},
  '國際水資源新聞': {cats:['國際'],                   color:'#4c1d95', grad:'linear-gradient(135deg,#3b0764,#7c3aed)'}
};
let activeCatGroup = null; // null = 全部

// localStorage keys
const LS_CORRECTIONS = 'wm_v5_corrections';
const LS_TRACKED     = 'wm_v5_tracked';
const LS_WORKFLOWS   = 'wm_v5_workflows';
...
function toROC(s) {
  if (!s) return '';
  const d = new Date(s);
  if (!isNaN(d)) return `民國${d.getFullYear()-1911}年${d.getMonth()+1}月${d.getDate()}日`;
  const d2 = new Date(s.slice(0,10));
  if (!isNaN(d2)) return `民國${d2.getFullYear()-1911}年${d2.getMonth()+1}月${d2.getDate()}日`;
  return s.slice(0,10);
}
...
function renderDonut(groupName) {
  ...
  el.innerHTML = mksvg(pn,'#ef4444','🔴 負面')+mksvg(pu,'#94a3b8','⚪ 中立')+mksvg(pp,'#22c55e','🟢 正面');
}
...
function renderCatCircles() {
  ...
  el.innerHTML = Object.entries(CAT_GROUPS).map(([name, cfg])=>{
    ...
    const sz = Math.round(80 + ratio * 50); // 80–130px
    const fs = Math.round(sz/6.5);
    return `<div class="cat-circle" id="cc-${name}" onclick="selectCat('${name}',this)">
      <div class="cat-ring" style="width:${sz}px;height:${sz}px;background:${cfg.grad}">
        <div style="font-size:${fs+4}px;font-weight:900">${count}</div>
        <div style="font-size:${Math.max(10,fs-2)}px;opacity:.85">則</div>
      </div>
...
/* ══════════════════════════════════════════
   今日緊急預警
══════════════════════════════════════════ */
function renderUrgent(latest) {
  const items = (MONITOR_DATA[latest]?.items||[]).filter(x=>x.priority==='高'&&getSentiment(x)==='負面');
  const el = document.getElementById('urgentList');
  if (!items.length) {
    el.classList.add('hidden');
    document.getElementById('urgentEmpty').classList.remove('hidden'); return;
  }
  ...
  el.innerHTML = items.slice(0,4).map((item,i)=>{
    let domain='';
    try{ domain = new URL(item.url||'').hostname; }catch(e){}
    const favicon = domain ? `https://www.google.com/s2/favicons?sz=64&domain=${domain}` : '';
    return `<div class="urg-card flex gap-0 overflow-hidden">
      ${favicon?`<div style="width:56px;min-width:56px;background:linear-gradient(135deg,#1a3a6c,#1e5799);display:flex;align-items:center;justify-content:center;padding:.5rem">
        <img src="${favicon}" class="w-8 h-8 rounded" onerror="this.style.display='none'">
      </div>`:''}
      <div class="flex-1 p-2.5 min-w-0">
        <a href="${item.url||'#'}" target="_blank" rel="noopener"
           class="link-title text-xs font-semibold block mb-1 leading-snug line-clamp-2">${item.title||''}</a>
        <div class="flex items-center gap-2 text-xs text-slate-400">
          <span>${item.source||''}</span>
          <span>${toROC(item.published)}</span>
          ${item.line_message?`<button onclick="cpUrgent('ul-${i}',this)"
            class="ml-auto text-xs bg-green-500 text-white px-2 py-0.5 rounded-full font-medium">📋</button>`:''}
        </div>
      </div>
...
function renderClarifyList() {
  ...
  const stages = ['AI文稿','人工修正','長官同意','確認發布','歸檔'];
...
function wfAction(id, stage) {
  ...
  if (stage === 5) { exportSingleCSV(id); return; }
  ...
  document.getElementById('wfModalContent').innerHTML = `
    ...
    <div class="mb-4">
      ...
      <textarea id="wfTextarea" rows="6"
        class="w-full border border-slate-200 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
        placeholder="輸入或修正澄清文稿…">${text}</textarea>
    </div>
...
function exportCSV() {
  ...
  const rows = items.map(item=>{
    ...
    return [toROC(item.published),item.title||'',item.source||'',item.url||'',
      getSentiment(item), stages[w.stage||1]||'', w.edited_text||item.line_message||'']
      .map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',');
  });
  const csv='日期,標題,來源,網址,情感,進度,澄清文字\n'+rows.join('\n');
...
function exportSingleCSV(id) {
  const item = allItemsFlat.find(x=>getItemId(x)===id);
  if (!item) return;
  const wf = getWorkflows()[id]||{};
  const stages=['','AI文稿','人工修正','長官同意','確認發布','歸檔'];
  const row=[toROC(item.published),item.title||'',item.source||'',item.url||'',
    getSentiment(item),stages[wf.stage||1]||'',wf.edited_text||item.line_message||'']
    .map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',');
  dlFile('﻿日期,標題,來源,網址,情感,進度,澄清文字\n'+row,'輿情_'+id.slice(0,8)+'.csv','text/csv;charset=utf-8');
  const w=getWorkflows(); if(!w[id])w[id]={};w[id].stage=Math.max(w[id].stage||1,5);
  saveWorkflows(w); renderClarifyList();
}
  ...
  c.innerHTML=items.map((item,idx)=>{
    ...
    return `<div class="news-card p-3.5 border border-slate-100 ${bg} ${bc}" id="card-${idx}">
      ...
      <div class="mt-2.5 pt-2 border-t border-slate-100 flex items-center gap-2 flex-wrap">
        <span class="text-xs text-slate-400">${toROC(item.published)}</span>
        <span class="text-xs text-slate-300">點閱：-</span>
        <div class="flex items-center gap-1 ml-auto">
          ...
          <button class="track-btn ml-2 ${isTracked?'on':''}" onclick="toggleTrack('${id}',${idx})"
            id="tb-${idx}">${isTracked?'✓ 追蹤中':'+ 追蹤'}</button>
        </div>
...
function toggleTrack(id, idx) {
  ...
  if(btn){btn.textContent=t[id]?'✓ 追蹤中':'+ 追蹤'; btn.classList.toggle('on',!!t[id]);}
  renderClarifyList();
}
  ...
  el.innerHTML=items.slice(0,8).map(item=>{
    ...
    return `<div style="border-bottom:1px solid #1a2e4a" class="pb-2 last:border-0 last:pb-0">
      ...
      <div class="flex items-center gap-1.5 text-xs text-blue-300">
        <span style="background:${sbg};color:${sc}" class="px-1.5 py-0.5 rounded">${s}</span>
        <span class="truncate">${item.source||''}</span>
        <span class="ml-auto shrink-0">${toROC(item.published||item.date)}</span>
      </div>
    </div>`;
  }).join('');
}
...
function cpUrgent(id,btn){
  const el=document.getElementById(id); if(!el) return;
  navigator.clipboard.writeText(el.textContent.trim()).then(()=>{ const o=btn.textContent; btn.textContent='✓'; setTimeout(()=>btn.textContent=o,2000); });
}
