    ...
    <div class="flex justify-between items-center gap-4 flex-wrap">
      ...
      <div class="flex items-center gap-4 shrink-0 flex-wrap">
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <a href="https://github.com/adam7315/water-monitor/actions/workflows/daily_monitor.yml"
             target="_blank" rel="noopener"
             class="text-xs bg-orange-500 hover:bg-orange-600 text-white px-3 py-1.5 rounded-full font-medium flex items-center gap-1.5 shadow text-center justify-center transition">
            ⚡ 立即蒐集
          </a>
        </div>
      </div>
...
function applyFilters() {
  ...
  if (from) f = f.filter(x=>normalizeDate(x.published||x.date||'')>=from);
  if (to)   f = f.filter(x=>normalizeDate(x.published||x.date||'')<=to);
  ...
  if      (sortBy==='date-desc') f.sort((a,b)=>(normalizeDate(b.published||b.date||'')>normalizeDate(a.published||a.date||'')?1:-1));
  else if (sortBy==='date-asc')  f.sort((a,b)=>(normalizeDate(a.published||a.date||'')>normalizeDate(b.published||b.date||'')?1:-1));
