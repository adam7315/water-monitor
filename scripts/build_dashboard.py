...
function filterToday(sent) {
  ...
  panelList.innerHTML = items.map((item, i)=>{
    const id = getItemId(item);
    const s = getSentiment(item);
    const neg=s==='負面',pos=s==='正面';
    ...
    const clarify = neg&&item.line_message?`
      <div class="line-box p-3 mt-2.5">
        <div class="flex justify-between items-center mb-1.5">
          <span class="text-xs font-semibold text-green-700">📱 AI 澄清建議</span>
          <button onclick="cpText('fpcl-${i}',this)" class="text-xs bg-green-500 hover:bg-green-600 text-white px-3 py-0.5 rounded-full font-medium">複製</button>
        </div>
        <p id="fpcl-${i}" class="text-xs text-slate-700 leading-relaxed">${item.line_message}</p>
      </div>`:'';
    return `<div class="news-card p-3.5 border border-slate-100 ${bg} ${bc}" id="fp-${i}">
      ...
      <div class="mt-2.5 pt-2 border-t border-slate-100 flex items-center gap-2 flex-wrap">
        ...
        <div class="flex items-center gap-1 ml-auto">
          ...
          <button class="corr-btn corr-neg ${s==='負面'?'on':''}" onclick="fpCorr('${id}','負面',${i})">負面</button>
          <button class="corr-btn corr-pos ${s==='正面'?'on':''}" onclick="fpCorr('${id}','正面',${i})">正面</button>
          <button class="corr-btn corr-neu ${s==='中立'?'on':''}" onclick="fpCorr('${id}','中立',${i})">中立</button>
          <button class="track-btn ml-2 ${isTracked?'on':''}" id="fp-tb-${i}" onclick="fpTrack('${id}',${i})">${isTracked?'✓ 已關注':'+ 關注'}</button>
        </div>
      </div>
    </div>`;
