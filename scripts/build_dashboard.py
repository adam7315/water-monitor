  ...
  panelList.innerHTML = items.map((item, i)=>{
    ...
    return `<div class="news-card p-3.5 border border-slate-100 ${bg} ${bc}" id="fp-${i}">
      ...
      <div class="mt-2.5 pt-2 border-t border-slate-100 flex items-center gap-2 flex-wrap">
        <span class="text-xs text-slate-400">${toROC(item.published||item.date)}</span>
...
function renderTodayNeg(latest) {
  const items = (MONITOR_DATA[latest]?.items||[]).filter(x=>
    getSentiment(x)==='負面' && normalizeDate(x.published||x.date||'')===latest);
    ...
    return `<div class="urg-card flex gap-0 overflow-hidden" id="tdn-${i}">
      ...
      <div class="flex-1 p-2 min-w-0">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="text-xs text-slate-300">${toROC(item.published||item.date)}</span>
